...
def apply_flags(settings, flags: PipelineFlags):
    ...
    `ENABLE_COMPRESSION` (F8). `RETRIEVAL_MODE` (the eval-only override) stays untouched so a
    `bm25_only` diagnostic run still wins over the boolean flag. Same two call sites F5 wired
    (`baseline._pipeline_events` + `evals.retrieval.run_retrieval`) — F6/F7/F8 add no new seam."""
